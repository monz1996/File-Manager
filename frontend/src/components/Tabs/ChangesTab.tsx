    ...
    <div className="space-y-6">
      ...
      <div className="p-6 rounded-2xl bg-gradient-to-r from-slate-900 via-indigo-950/40 to-slate-900 border border-slate-800 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5 bg-slate-800/80 p-1 rounded-xl border border-slate-700 text-xs">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-medium transition cursor-pointer ${
                selectedType === 'all' ? 'bg-indigo-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-medium transition cursor-pointer ${
                selectedType === 'download_move' ? 'bg-indigo-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-medium transition cursor-pointer ${
                selectedType === 'drive_sync' ? 'bg-indigo-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-medium transition cursor-pointer ${
                selectedType === 'audit_rename' ? 'bg-indigo-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
          ...
          <button
            ...
            disabled={loading}
            className="px-3 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 text-xs font-medium transition cursor-pointer flex items-center gap-1.5"
          >
      ...
      {filteredOps.length === 0 ? (
        <div className="p-12 text-center rounded-2xl bg-slate-900/50 border border-slate-800/80">
          ...
          <h3 className="text-base font-semibold text-slate-300">No Operations Recorded Yet</h3>
            ...
            return (
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2.5">
                    {isReverted ? (
                      <span className="flex items-center gap-1 text-xs px-2.5 py-1 rounded-full bg-slate-800 text-slate-400 border border-slate-700 font-medium">
                ...
                <div className="mt-3">
                  ...
                  <div className="mt-2.5 grid grid-cols-1 md:grid-cols-2 gap-2 text-xs">
                    <div className="p-2 rounded-lg bg-slate-950/60 border border-slate-800/80">
                      <span className="text-[11px] font-semibold text-slate-400 block mb-0.5">Source Path:</span>
                      <code className="text-slate-300 break-all select-all font-mono">{op.source}</code>
                    </div>
                    <div className="p-2 rounded-lg bg-slate-950/60 border border-slate-800/80">
                      <span className="text-[11px] font-semibold text-slate-400 block mb-0.5">Destination Path:</span>
